...
export const add1000 = new Module({
  routeName: 'add1000',
  title: 'sčítání',
  subtitle: 'čísla do tisíce',
  onActivate: (ctx, options) => {
    onModuleActivate(ctx, options);
    return { timeLimit: 8000, maxResponseLen: 4 };
  },
  next: (ctx) => {
    let found = false;
    let a1;
    ...
    while (!found) {
      ...
      if (method === 2) {
        a1 = randomInt(1001); // 0 .. 1000
        a2 = randomInt(1001);
      }
      found = true;
      // make sure the result is up to 1000
      if (a1 + a2 > 1000) { found = false; }
